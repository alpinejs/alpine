...
test('two independent x-if/x-else chains do not cross-contaminate',
    html`
        <div x-data="{ a: true, b: false }">
            <template x-if="a">
                <span id="a-true">A true</span>
            </template>
            <template x-else>
                <span id="a-false">A false</span>
            </template>

            <template x-if="b">
                <span id="b-true">B true</span>
            </template>
            <template x-else>
                <span id="b-false">B false</span>
            </template>
        </div>
    `,
    ({ get }) => {
        get('#a-true').should('exist').and('contain', 'A true')
        get('#b-false').should('exist').and('contain', 'B false')
        get('#a-false').should(notExist())
        get('#b-true').should(notExist())
    }
);
