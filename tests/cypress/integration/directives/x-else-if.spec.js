...
test('x-else-if chain of 3+: only first matching branch shows when conditions overlap',
    html`
        <div x-data="{ value: 3 }">
            <button @click="value = 1">1</button>
            <button @click="value = 2">2</button>
            <button @click="value = 3">3</button>

            <template x-if="value >= 1">
                <h1 id="branch-1">Value >= 1</h1>
            </template>
            <template x-else-if="value >= 2">
                <h2 id="branch-2">Value >= 2</h2>
            </template>
            <template x-else-if="value >= 3">
                <h3 id="branch-3">Value >= 3</h3>
            </template>
            <template x-else>
                <h4 id="branch-else">Else</h4>
            </template>
        </div>
    `,
    ({ get }) => {
        // value is 3: only first branch (value >= 1) must show; later else-ifs must not show
        get('#branch-1').should('exist').and('contain', 'Value >= 1')
        get('#branch-2').should(notExist())
        get('#branch-3').should(notExist())
        get('#branch-else').should(notExist())

        get('button').contains('2').click()
        get('#branch-1').should('exist')
        get('#branch-2').should(notExist())
        get('#branch-3').should(notExist())

        get('button').contains('1').click()
        get('#branch-1').should('exist')
        get('#branch-2').should(notExist())
        get('#branch-3').should(notExist())

        get('button').contains('3').click()
        get('#branch-1').should('exist')
        get('#branch-2').should(notExist())
        get('#branch-3').should(notExist())
    }
