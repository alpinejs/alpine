...
test('click.{systemKey}.exact single/combo modifier', async () => {
    document.body.innerHTML = `
        <div x-data="{ count: 0 }">
            <button
...
test('click.exact with no system key modifier', async () => {
    document.body.innerHTML = `
        <div x-data="{ count: 0 }">
            <button
                x-on:click.exact="count++"
            ></button>
