...
> ملاحظة: يمكنك اختصار الكتابة باستعمال النقطتين ":" مثل `@click="..."`
...
> يمكننا كذلك الإستجابة لأزرار لوحة المفاتيح الأساسية مثل `x-on:keydown.cmd.enter="foo"`
...
تؤدي إضافة `.prevent` داخل مستمع الحدث إلى استدعاء منع `preventDefault` في الحدث الذي سيتم تنفيذه. في المثال أعلاه، هذا يعني أن مربع الاختيار لن يتم تحديده بالفعل عندما ينقر المستخدم عليه.

**المُعدّل`.stop`**
**المثال::** `<div x-on:click="foo = 'bar'"><button x-on:click.stop></button></div>`

إضافة `.stop` إلى المنصت للأحداث يستدعي `stopPropagation.` في المثال أعلاه، يعني أن الحدث "click" لن ينتقل إلى  `<div>` الخارجي. بمعنى آخر، عندما ينقر المستخدم على الزر، لا يتم تعريف `foo` على أنه `bar`.

**المُعدّل `.self`**
**المثال:** `<div x-on:click.self="foo = 'bar'"><button></button></div>`

إضافة .self إلى المنصت للأحداث يأدي إلى تشغيل الحدث فقط إذا كان `$event.target` هو نفس العنصر. في المثال أعلاه، يعني أنه عند النقر على الزر "button" لن يتم تشغيل الحدث.

**المُعدّل `.window`**
**المثال:** `<div x-on:resize.window="isOpen = window.outerWidth > 768 ? false : open"></div>`

إضافة `.window` إلى منصت الأحداث، سيقوم بتثبيت منصت للأحداث على نافذة المتصفّح كله `window` بدلاً من DOM الذي قمت بتصريحه أو تحديده. هذا مفيد لك في حال أردت ضبط أحد المكوّنات وتغييرها على حسب حجم (أبعاد) المتصفّح. في المثال أعلاه سيتم إغلاق النافذة أو القائمة المنسدلة إذا تجاوزت أبعاد المتصفح 768 بكسل، خلاف ذلك نُبقيه على حالته.
...
تعني إضافة المعدّل `.once` إلى المنصت للحدث أن المنصت (listener) يعمل مرة واحدة فقط. هذا مفيد للمهام التي تريد القيام بها مرة واحدة فقط، مثل الجلب الجزئي لشفرات HTML أو ما شابه.

**المُعدّل `.passive` **
**المثال:** `<button x-on:mousedown.passive="interactive = true"></button>`

إذا أضفنا `.passive` إلى المنصت للحدث، فإن هذا الرمز المميز سيعطل وظيفة `preventDefault()` ولن تعمل على أي حدث يتم تنفيذه. يمكن أن يساعدك أحياناً في تحسين أداء التمرير (scroll) على الأجهزة التي تعمل باللمس.
...
يقوم `.number` بتحويل القيمة المدخلة عبر `<input>` إلى رقم. في حال تعذّر تحليل القيمة المدخلة لرقم فعلاً سيُرجع القيمة الأصلية المدخلة.
...
> يمكنك أيضا ربط القيم المتغيّرة (الديناميكية) بـ `<span :x-ref="item.id"></span>` .
...
> - الحالة الخاصة الوحيدة لـ spread هي استخدامه مع `x-for`. عند تطبيقه على التوجيه `x-for`، يجب إرجاع تعليمات نصيّة عادية بواسطة callback.  مثال `['x-for']() { return 'item in items' }`
...
تمكّنك `$watch` في المثال أعلاه من تفعيل `open` عند الضغط على الزر "button"، يقوم callback بتنفيذ `console.log` وطباعة القيمة الجديدة.
