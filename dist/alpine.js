(function (global, factory) {
    ...
    function saferEval(expression, dataContext, additionalHelperVariables = {}) {
      return new Function(['$data', '$extraVars'], `var result; with($extraVars) { with($data) { result = ${expression} } }; return result`)(dataContext, additionalHelperVariables);
    }
    function saferEvalNoReturn(expression, dataContext, additionalHelperVariables = {}) {
      // For the cases when users pass only a function reference to the caller: `x-on:click="foo"`
      // Where "foo" is a function. Also, we'll pass the function the event instance when we call it.
      if (Object.keys(dataContext).includes(expression)) {
        let methodReference = new Function(['dataContext', '$extraVars'], `with($extraVars) { with(dataContext) { return ${expression} } }`)(dataContext, additionalHelperVariables);
      ...
      return new Function(['dataContext', '$extraVars'], `with($extraVars) { with(dataContext) { ${expression} } }`)(dataContext, additionalHelperVariables);
    }
